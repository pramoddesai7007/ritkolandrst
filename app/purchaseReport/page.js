...
const PurchaseReport = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const purchasesResponse = await axios.get("https://absback1.vercel.app/api/purchase/purchases");
        setPurchases(purchasesResponse.data);
        setFilteredPurchases(purchasesResponse.data);

        const itemsResponse = await axios.get("https://absback1.vercel.app/api/item/items");
        setItems(itemsResponse.data);

        const vendorsResponse = await axios.get("https://absback1.vercel.app/api/supplier/suppliers");
    ...
    <>
      ...
      <div className="container mx-auto p-4 mt-20">
        <div className="flex flex-wrap items-center mb-4 justify-center text-left">
          ...
          />
          <button
            className="text-orange-600 font-bold py-1 rounded-full text-sm bg-orange-100 mr-2 px-2 shadow-md ml-3"
