...
const Billing = ({ tableId, acPercentage }) => {
    ...
    const fetchTastes = async () => {
      try {
        const response = await axios.get(
          "https://absback1.vercel.app/api/taste/tastes"
        );
  ...
  const kotItemsContent = modifiedCurrentOrder
    .map(
      (orderItem, index) => `
  <tr>
    <td>${index + 1}</td>
    <td class="kot-item-name">${orderItem.name}</td>
    <td class="kot-taste">${orderItem.selectedTaste ? orderItem.selectedTaste.taste : "Not selected"
        }</td>
...
`
  ...
  const fetchWaitersList = async () => {
    try {
      const response = await axios.get("https://absback1.vercel.app/api/waiter");
  ...
  useEffect(() => {
    const fetchGreetings = async () => {
      try {
        const response = await axios.get(
          "https://absback1.vercel.app/api/greet/greet"
        );
  ...
  useEffect(() => {
    ...
    const fetchNextOrderNumber = async () => {
      try {
        const response = await axios.get(
          "https://absback1.vercel.app/api/order/get-next-order-number"
        );
  ...
  const fetchLastAllOrders = async () => {
    try {
      const ordersResponse = await axios.get(
        "https://absback1.vercel.app/api/order/latest-orders"
      );
      ...
      const tableNamesPromises = orders.map(async (order) => {
        const tableResponse = await axios.get(
          `https://absback1.vercel.app/api/table/tables/${order.tableId}`
        );
  ...
  const saveBill = async () => {
    try {
      ...
      const orderData = {
        tableId,
        waiterName,
        // tastes:orderItem.selectedTaste.taste,
      ...
      const existingBillResponse = await axios.get(
        `https://absback1.vercel.app/api/order/order/${tableId}`
      );
      ...
      if (existingBill && existingBill.length > 0) {
        ...
        const updateResponse = await axios.patch(
          `https://absback1.vercel.app/api/order/update-order-by-id/${orderIdToUpdate}`,
          orderData
        );
      ...
      const KOTResponse = await axios.post(
        `https://absback1.vercel.app/api/kot/kotOrder/${tableId}`,
        kotData
      );
  ...
  ${[...uniqueItemsSet]
        ...
        <tr>
          <td>${index + 1}</td>
          <td class="kot-item-name">${orderItem.name}${tasteInfo && tasteInfo.selectedTaste
  ...
  const saveKot = async () => {
    try {
      ...
      const existingKOTResponse = await axios.get(
        `https://absback1.vercel.app/api/kot/kot/${tableId}`
      );
        ...
        <html>
          ...
          .map(
            (orderItem, index) => `
                <tr>
                  <td>${index + 1}</td>
                  <td class="kot-item-name">${orderItem.name}${orderItem.selectedTaste
                ? `<br>Taste - ${orderItem.selectedTaste.taste}`
                : ""
              }</td>
                  <td>${orderItem.quantity}</td>
                </tr>
...
<p style="${!hotelInfo || !hotelInfo.sacNo ? "display: none;" : ""}">SAC No: ${hotelInfo ? hotelInfo.sacNo : "SAC No Not Found"
  ...
  useEffect(() => {
    ...
    // Fetch categories
    axios
      .get("https://absback1.vercel.app/api/main")
      .then((response) => {
    ...
    // Fetch products
    axios
      .get("https://absback1.vercel.app/api/menu/menus/list")
      .then((response) => {
        console.log(response.data);
    ...
    if (tableId) {
      axios
        .get(`https://absback1.vercel.app/api/table/tables/${tableId}`)
        .then((response) => {
  ...
  useEffect(() => {
    ...
    if (selectedCategory) {
      axios
        .get(`https://absback1.vercel.app/api/menu/${selectedCategory._id}`)
        .then((response) => {
          console.log(response.data);
          const menusArray = response.data || []; // Ensure menus is an array
          setMenus(menusArray);
  ...
  const handleCategoryClick = (category) => {
    ...
    if (category === null) {
      ...
      // Fetch menus based on the selected category
      axios
        .get(`https://absback1.vercel.app/api/menu/menulist/${category._id}`)
        .then((response) => {
          setMenus(response.data);
        })
        .catch((error) => {
    ...
    const fetchHotelInfo = async () => {
      try {
        ...
        const allHotelsResponse = await axios.get(
          "https://absback1.vercel.app/api/hotel/get-all"
        );
        ...
        if (defaultHotelId) {
          ...
          const response = await axios.get(
            `https://absback1.vercel.app/api/hotel/get/${defaultHotelId}`
          );
    ...
    <div className=" font-sans  ">
      ...
      <div className="container mx-auto bg-white">
        <div className="flex lg:flex-row shadow-lg ">
          {/* <!-- left section --> */}
          <div className="w-1/2 lg:w-2/3 min-h-screen shadow-lg">
            {/* <!-- header --> */}
            ...
            <div className="cursor-pointer grid grid-cols-2 bg-white  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 px-5 mt-3 custom-sidescrollbars overflow-scroll max-h-[calc(90vh-2rem)]">
              {(menus.menus || menus)
                    ...
                    <div>
                      <div>
                        ...
                        <div className="font-medium text-yellow-500">
                          ...
                          <span className="text-sm">{`${product.uniqueId || "Yes"
                            }`}</span>
                        </div>
                      </div>
                    </div>
          ...
          <div className=" w-1/2 lg:w-5/8 bg-gray-200">
            {/* Header */}
            <div className="font-bold text-sm px-3 flex mt-1">
              Last Bills
              <div className="mt-1 mb-1 ml-2 float-right">
                <input
                  ...
                  className="p-1 rounded-full ml-1 px-1  sm:w-32 md:w-48 lg:w-28 pl-2 font-medium"
                />
              </div>
            </div>
            ...
            <div className="font-bold text-xl px-5 flex">
              ...
              <select
                ...
                className="mt-1 p-1 border rounded-md text-xs text-gray-400 ml-4"
                required
              >
            ...
            {/* <!-- end header --> */}
            {/* <!-- order list --> */}
            <div className="p-2 custom-scrollbars overflow-y-auto lg:h-64 md:h-48">
              {currentOrder.map((orderItem) => (
                <div key={orderItem._id} className="flex flex-col md:flex-row items-center mb-3">
                  <div className="flex items-center h-full mb-2 md:mb-0">
                    <img
                      src={`/tray.png`}
                      className="w-6 h-6 object-cover rounded-md"
                      alt=""
                    />
                    <span className="ml-2 font-semibold text-sm w-28 lg:text-base">
                      {orderItem.name}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row items-center">
                    ...
                    Select
                    <input
                      list={`tasteSelect_${orderItem._id}`}
                      name={`tasteSelect_${orderItem._id}`}
                      value={selectedTastes[orderItem._id] || ""}
                      onChange={(e) =>
                        handleSelectChange(orderItem._id, e.target.value)
                      }
                      className="mt-1 p-1 border rounded-md text-sm text-gray-500"
                      required
                    />
                  </label>
                  <datalist id={`tasteSelect_${orderItem._id}`}>
                  ...
                  {/* {selectedTastes[orderItem._id] === "other" && (
                    ...
                    <div className="flex justify-between items-center mt-1 md:mt-0">
                      <span
                        className="rounded-md cursor-pointer  bg-indigo-300 lg:px-2 font-bold"
                      ...
                      <input
                        ...
                        className="font-semibold w-8 text-center rounded-md align-center md:mx-2"
                      />
                      <span
                        className="rounded-md cursor-pointer  bg-indigo-300 lg:px-1 font-bold"
                  ...
                  <div className="font-semibold text-sm lg:text-base text-right mt-2 md:mt-0 ml-2">
