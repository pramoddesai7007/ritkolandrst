...
const EditOrderPage = ({ params, tableId }) => {
      ...
      try {
        if (orderNumber) {
          ...
          const orderResponse = await axios.get(
            `https://absback1.vercel.app/api/order/get/order/${orderNumber}`
          );
          ...
          const sectionResponse = await axios.get(
            `https://absback1.vercel.app/api/section/sectionlist/${tableId}`
          );
  ...
  const handleSaveBill = async () => {
    try {
      ...
      await axios.post(`https://absback1.vercel.app/api/logHistory/log-history`, {
      ...
      const updateOrderEndpoint = `https://absback1.vercel.app/api/order/update-order-by-number/${orderNumber}`;
  ...
  const handlePrintBill = async () => {
    try {
      ...
      const tableResponse = await axios.get(
        `https://absback1.vercel.app/api/table/tables/${tableId}`
      );
