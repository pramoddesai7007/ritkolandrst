...
const PaymentList = () => {
    ...
    useEffect(() => {
        const fetchPayments = async () => {
            try {
                setLoading(true);
                const response = await axios.get('https://absback1.vercel.app/api/payment/paymentsList');
                const paymentsWithTableNames = await Promise.all(
                    response.data.map(async (payment) => {
                        const tableResponse = await axios.get(`https://absback1.vercel.app/api/table/tables/${payment.tableId}`);
                        return {
                            ...payment,
            ...
            <div className="container mx-auto p-8 bg-white rounded-md shadow-md font-sans mt-4">
                ...
                {loading ? (
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className=" bg-zinc-100 text-yellow-700 border">
                            <tr>
                                <th className="px-6 py-3  text-center text-xs leading-4 font-lg  uppercase tracking-wider">
                                    SR No
                                </th>
                                <th className="px-6 py-3  text-center text-xs leading-4 font-lg  tracking-wider">
                                    Bill Number
                                </th>
                                <th className="px-6 py-3  text-center text-xs leading-4 font-lg   tracking-wider">
                                    Bill Date
